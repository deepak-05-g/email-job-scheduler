...
import { getScheduledEmails } from '../lib/api-client.js';
...
export const ScheduledEmailsPage: React.FC = () => {
  ...
  const fetchScheduled = async (page = 1) => {
    ...
    setError(null);
    try {
      const response = await getScheduledEmails(page, 25);
      setEmails(response.data);
      setPagination(response.pagination);
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Failed to load scheduled emails';
      setError(msg);
    } finally {
      setLoading(false);
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            Monitor and audit delayed email jobs waiting for dispatch in BullMQ.
          </p>
        </div>
      ...
      <div className="bg-white border border-gray-200/90 rounded-2xl overflow-hidden shadow-2xs">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <h3 className="text-xs font-semibold text-gray-800">No scheduled emails in queue</h3>
            <p className="text-[11px] text-gray-400 mt-0.5 max-w-sm mx-auto">
              All scheduled jobs have been dispatched by the background worker.
            </p>
          </div>
        ) : (
          <>
            ...
            <Pagination
              ...
              onPageChange={(page) => fetchScheduled(page)}
              disabled={loading}
            />
          </>
        )}
